...
class MATRewardConfig:
    ...
    def __post_init__(self):
        if self.objective_mode not in {"delay_only", "legacy_delay_kl"}:
            raise ValueError("objective_mode must be delay_only or legacy_delay_kl")
...
def compute_mat_reward(system_max_delay, label_distributions, cluster_choices, bandwidths, config, ideal_distribution=None):
    """Compute the formal delay reward and label-heterogeneity diagnostics.

    Label KL is deliberately diagnostic-only in the default mode.  The legacy
    scalarisation remains available solely for reading/replaying older runs.
    """
    ...
    kl_sum = 0.0
    weighted_kl_sum = 0.0
    station_distribution = labels.mean(axis=0)
    station_distribution /= station_distribution.sum()
    station_distribution = np.clip(station_distribution, config.epsilon, None)
    station_distribution /= station_distribution.sum()
    for cluster_id in cluster_ids:
        ...
        weighted_kl_sum += float(len(members) / len(labels)) * float(np.sum(
            cluster_distribution[positive] * np.log(
                cluster_distribution[positive] / station_distribution[positive])))
    ...
    objective = (
        delay_normalized
        if config.objective_mode == "delay_only"
        else config.delay_weight * delay_normalized + (1.0 - config.delay_weight) * kl_normalized
    )
    ...
    return -(objective + penalty), {
        ...
        "label_kl_weighted_to_station": weighted_kl_sum,
        "reward_objective_mode": config.objective_mode,
