...
class MATAgent(BaseAgent):
    def __init__(self, state_dim, hidden_dim=128, num_migs=7, num_cut_layers=7, edge_state_dim=2,
        ...
        actor_parameters = list(self.device_decoder.parameters()) + list(self.split_head.parameters())
        if self.bandwidth_policy == "joint_dirichlet":
            actor_parameters += list(self.bandwidth_head.parameters())
    ...
    def _parameters(self):
        modules = [self.encoder, self.device_decoder, self.split_head, self.value_head]
        if self.bandwidth_policy == "joint_dirichlet":
            modules.append(self.bandwidth_head)
    ...
    def _bandwidth_context(self, prepared_state, edge_state):
        masked = prepared_state.clone()
    ...
    def act(self, active_clients_state, available_migs, edge_state, client_ids=None, deterministic=False):
        ...
        if self.bandwidth_policy == "joint_dirichlet":
            ...
            (bandwidths, joint_lp, joint_entropy, alpha, context_score,
             physical_score, physical_feature) = self.bandwidth_head.act(
                context, clusters, prepared_state[..., :1], deterministic=deterministic)
        ...
        policy_info = {
            ...
            "bandwidth_alpha": alpha.squeeze(0).cpu().numpy(),
    ...
    def evaluate_bandwidth_prefix_means(
        ...
        means = evaluated["bandwidth_latent_means"].reshape(1, -1)
        if self.bandwidth_policy == "joint_dirichlet":
            return means.squeeze(0).cpu().numpy()
    ...
    def _evaluate_action(self, state, edge_state, action, available_migs, decision_order):
        ...
        if self.bandwidth_policy == "joint_dirichlet":
            ...
            context = self._bandwidth_context(prepared_state, edge_state)
            (joint_lp, joint_entropy, alpha, context_score,
             physical_score, physical_feature) = self.bandwidth_head.evaluate_actions(
                context, clusters, prepared_state[..., :1], bandwidths)
            bandwidth_lp = joint_lp.unsqueeze(-1)
            bandwidth_entropy = joint_entropy.unsqueeze(-1)
            ...
            ordered_channels = None if channels is None else channels[:, order]
        ...
        return {"cluster_log_probs": cluster_lp.squeeze(0), "bandwidth_log_probs": bandwidth_lp.squeeze(0),
                ...
                "bandwidth_alpha": alpha.squeeze(0), "bandwidth_context_scores": context_score.squeeze(0),
                "bandwidth_physical_scores": physical_score.squeeze(0),
    ...
    def _compute_gae(self, rewards, next_states, dones, edge_states, next_edge_states, policy_infos,
        ...
        for trajectory in dict.fromkeys(trajectories):
            ...
            for index in indices:
                nonterminal = 0.0 if (dones[index] or one_step) else 1.0
    ...
    def _sample_terms(self, index, states, actions, edge_states, available_migs, policy_infos, advantages, td_targets):
        ...
        component_pairs = {name: pair for name, pair in component_pairs.items()
                           if name in self._active_actor_components}
        component_results = {
        ...
        entropy_parts = []
        if "cluster" in self._active_actor_components:
            entropy_parts.append(evaluated["cluster_entropy"].reshape(-1))
        if "bandwidth" in self._active_actor_components and bandwidth_mask.any():
            entropy_parts.append(evaluated["bandwidth_entropy"][bandwidth_mask].reshape(-1))
        if "split" in self._active_actor_components and split_mask.any():
            entropy_parts.append(evaluated["split_entropy"][split_mask].reshape(-1))
        entropy = torch.cat(entropy_parts).mean() if entropy_parts else value_loss.detach() * 0.0
    ...
    def _microbatch_terms(self, indices, states, actions, edge_states, available_migs,
        ...
        if self.bandwidth_policy == "joint_dirichlet":
            ...
            masked[..., 0] = self.bandwidth_context_channel_reference
            context = self.encoder(masked, edges)
            joint_lp, joint_entropy, _, _, _, _ = self.bandwidth_head.evaluate_actions(
        ...
        for row, index in enumerate(indices):
            ...
            pairs = {name: pair for name, pair in pairs.items() if name in self._active_actor_components}
            ...
            entropy_parts = []
            if "cluster" in self._active_actor_components:
                entropy_parts.append(cluster_entropy[row].reshape(-1))
            if "bandwidth" in self._active_actor_components and bandwidth_mask.any():
                entropy_parts.append(bandwidth_entropy[row][bandwidth_mask].reshape(-1))
            if "split" in self._active_actor_components and split_mask.any():
                entropy_parts.append(split_entropy[row][split_mask].reshape(-1))
            entropy = torch.cat(entropy_parts).mean() if entropy_parts else value_loss.detach() * 0.0
            zero = policy_loss.detach() * 0.0
    ...
    def update_policy(self, rewards, next_states, dones, **kwargs):
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, count, self.minibatch_size):
                ...
                if start == 0:
                    for component in component_names:
                        if component in self._active_actor_components:
        ...
        for info in data["policy_infos"]:
            means = np.asarray(info["bandwidth_latent_means"])
            if len(info["bandwidth_mask"]) == len(means):
                means = means[np.asarray(info["bandwidth_mask"], dtype=bool)]
        ...
        context_scores = np.concatenate([np.asarray(info["bandwidth_context_scores"]).reshape(-1)
                                         for info in data["policy_infos"]])
        alphas = np.concatenate([np.asarray(info["bandwidth_alpha"]).reshape(-1)
                                 for info in data["policy_infos"]])
        result["bandwidth_alpha_mean"] = float(alphas.mean())
        result["bandwidth_alpha_min"] = float(alphas.min())
