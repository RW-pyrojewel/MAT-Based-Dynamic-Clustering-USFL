...
class MATChannelConditioningTests(unittest.TestCase):
    ...
    def test_channel_sidecar_has_gradient_and_counterfactual_response(self):
        ...
        with torch.no_grad():
            agent.device_decoder.bandwidth_channel_head.weight.fill_(-2.0)
        ...
        prepared, encoded = agent._encode(self.state, self.edge)
        order = torch.arange(10)
        _, _, _, means, _, _ = agent.device_decoder.evaluate_actions(
            encoded,
            torch.as_tensor(action["cluster"]).reshape(1, -1),
            torch.as_tensor(action["bw"]).reshape(1, -1),
            3,
            channel_features=agent._channel_features(prepared)[:, order],
            return_diagnostics=True,
        )
        gradient = torch.autograd.grad(means.sum(), agent.device_decoder.bandwidth_channel_head.weight)[0]
    ...
    def test_checkpoint_round_trip(self):
        ...
        with torch.no_grad():
            agent.device_decoder.bandwidth_channel_head.weight.fill_(-0.75)
